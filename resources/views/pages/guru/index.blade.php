...
@push('scripts')
    <script>
        ...
        function loadGuru() {
            $('#guruTableBody').html(
                '<tr><td colspan="9" class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary"></div><span class="ms-2 text-muted">Memuat data...</span></td></tr>'
                );
            $.ajax({
                url: GURU_DATA,
                data: {
                ...
                error: function() {
                    $('#guruTableBody').html(
                        '<tr><td colspan="9" class="text-center text-danger py-3">Gagal memuat data.</td></tr>'
                        );
                }
            });
        }
        ...
        function renderGuru(data, meta) {
            if (!data.length) {
                $('#guruTableBody').html(
                    '<tr><td colspan="9" class="text-center text-muted py-4">Tidak ada data guru.</td></tr>');
                $('#guruInfo').text('');
                $('#guruPagination').html('');
                return;
            }
        ...
        function renderPaginationGuru(c, l) {
            ...
            let h =
                `<li class="page-item ${c===1?'disabled':''}"><a class="page-link" href="#" onclick="return guruGoPage(${c-1})">‹</a></li>`;
            for (let p = 1; p <= l; p++) {
                if (p === 1 || p === l || (p >= c - 1 && p <= c + 1)) h +=
                    `<li class="page-item ${p===c?'active':''}"><a class="page-link" href="#" onclick="return guruGoPage(${p})">${p}</a></li>`;
                else if (p === c - 2 || p === c + 2) h +=
                    `<li class="page-item disabled"><span class="page-link">…</span></li>`;
            }
            h +=
            `<li class="page-item ${c===l?'disabled':''}"><a class="page-link" href="#" onclick="return guruGoPage(${c+1})">›</a></li>`;
            $('#guruPagination').html(h);
        }

        function guruGoPage(p) {
            guruPage = p;
            loadGuru();
            return false;
        }
        ...
        function showGuru(id) {
            $.get(`${GURU_URL}/${id}`, function(res) {
                if (!res.success) return;
                const d = res.data;
                let avatar = '{{ asset('assets/images/users/guru/') }}';
        ...
        $('#confirmDeleteGuruBtn').on('click', function() {
            ...
            $('#btnDeleteGuruLoader').removeClass('d-none');
            $.ajax({
                ...
                error: function() {
                    showToastGuru('Gagal menghapus.', 'danger');
                },
                complete: function() {
                    $('#confirmDeleteGuruBtn').prop('disabled', false);
                    $('#btnDeleteGuruLoader').addClass('d-none');
                    deleteGuruId = null;
                }
            });
        });
        ...
        $('#addGuruForm').on('submit', function(e) {
            e.preventDefault();
            $('#btnSubmitAddGuru').prop('disabled', true);
            $('#btnLoaderAddGuru').removeClass('d-none');
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    $('#addGuruError').text(e ? Object.values(e).flat().join(', ') :
                        'Terjadi kesalahan.').removeClass('d-none');
                },
                complete: function() {
                    $('#btnSubmitAddGuru').prop('disabled', false);
                    $('#btnLoaderAddGuru').addClass('d-none');
                }
            });
        });
        ...
        $('#editGuruForm').on('submit', function(e) {
            ...
            $('#btnSubmitEditGuru').prop('disabled', true);
            $('#btnLoaderEditGuru').removeClass('d-none');
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let e = xhr.responseJSON?.errors;
                    $('#editGuruError').text(e ? Object.values(e).flat().join(', ') :
                        'Terjadi kesalahan.').removeClass('d-none');
                },
                complete: function() {
                    $('#btnSubmitEditGuru').prop('disabled', false);
                    $('#btnLoaderEditGuru').addClass('d-none');
                }
            });
        });
        ...
        function showToastGuru(msg, type) {
            let t = $(
                `<div class="position-fixed bottom-0 end-0 p-3" style="z-index:9999"><div class="toast show align-items-center text-white ${type==='success'?'bg-success':'bg-danger'} border-0"><div class="d-flex"><div class="toast-body">${msg}</div><button type="button" class="btn-close btn-close-white me-2 m-auto"></button></div></div></div>`
                );
            $('body').append(t);
