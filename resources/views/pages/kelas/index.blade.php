    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="d-flex align-items-center justify-content-between mb-3">
                ...
                {{-- ✅ Tidak pakai onclick / data-bs-toggle, pakai id saja --}}
                <button class="btn btn-primary btn-sm" id="btnBukaTambahKelas">
...
@push('scripts')
    <script>
        ...
        function loadData() {
            $('#tabelBody').html(
                '<tr><td colspan="8" class="text-center py-4"><div class="spinner-border spinner-border-sm text-primary"></div><span class="ms-2 text-muted">Memuat data...</span></td></tr>'
                );
            $.ajax({
        ...
        function renderTable(res) {
            if (!res.data.length) {
                $('#tabelBody').html(
                    '<tr><td colspan="8" class="text-center text-muted py-4">Tidak ada data kelas.</td></tr>');
                $('#tableInfo').text('');
            ...
            res.data.forEach(function(k, i) {
                let badge = {
                    'X': 'bg-soft-success text-success',
                    'XI': 'bg-soft-warning text-warning',
                    'XII': 'bg-soft-primary text-primary'
                } [k.tingkat] || 'bg-soft-secondary';
                html += `<tr>
            ...
            $('#tableInfo').text(`Menampilkan ${start}–${to} dari ${res.total} kelas`);
        ...
        function renderPagination(c, l) {
            ...
            let h =
                `<li class="page-item ${c===1?'disabled':''}"><a class="page-link" href="#" onclick="return goPage(${c-1})">‹</a></li>`;
            for (let p = 1; p <= l; p++) {
                if (p === 1 || p === l || (p >= c - 1 && p <= c + 1)) h +=
                    `<li class="page-item ${p===c?'active':''}"><a class="page-link" href="#" onclick="return goPage(${p})">${p}</a></li>`;
                else if (p === c - 2 || p === c + 2) h +=
                    `<li class="page-item disabled"><span class="page-link">…</span></li>`;
            }
            h +=
            `<li class="page-item ${c===l?'disabled':''}"><a class="page-link" href="#" onclick="return goPage(${c+1})">›</a></li>`;
        ...
        $('#addKelasForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    $('#addKelasError').html(e ? Object.values(e).flat().join('<br>') :
                        'Terjadi kesalahan.').removeClass('d-none');
                },
                complete: function() {
                    $('#btnSubmitAddKelas').prop('disabled', false);
                    $('#btnLoaderAddKelas').addClass('d-none');
                }
            });
        });
        ...
        $('#editKelasForm').on('submit', function(e) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    let e = xhr.responseJSON?.errors;
                    $('#editKelasError').html(e ? Object.values(e).flat().join('<br>') :
                        'Terjadi kesalahan.').removeClass('d-none');
                },
                complete: function() {
                    $('#btnSubmitEditKelas').prop('disabled', false);
                    $('#btnLoaderEditKelas').addClass('d-none');
                }
            });
        });
        ...
        function confirmDelete(id, nama, jml) {
            if (jml > 0) {
                showToast(`Kelas "${nama}" tidak bisa dihapus, masih ada ${jml} siswa.`, 'danger');
                return;
            }
            deleteId = id;
            $('#delete_kelas_name').text(nama);
        ...
        $('#confirmDeleteKelasBtn').on('click', function() {
            ...
            $('#btnDeleteKelasLoader').removeClass('d-none');
            $.ajax({
                url: '{{ url('admin/kelas') }}/' + deleteId,
                method: 'POST',
                data: {
                    _method: 'DELETE',
                    _token: '{{ csrf_token() }}'
                ...
                complete: function() {
                    $('#confirmDeleteKelasBtn').prop('disabled', false);
                    $('#btnDeleteKelasLoader').addClass('d-none');
                    deleteId = null;
                }
            });
        });
        ...
        $(document).ready(function() {
            ...
            $('#btnBukaTambahKelas').on('click', function() {
                let form = document.getElementById('addKelasForm');
                if (form) form.reset();
                $('#addKelasError').addClass('d-none');
