{{-- Ganti hanya bagian JS btnUpdateStatus di petugas_index.blade.php --}}
{{-- Cari baris ini: --}}
{{--     method: 'POST', --}}
{{--     data: { --}}
{{--         _token:              '{{ csrf_token() }}', --}}
{{--         _method:             'PUT',   <-- HAPUS BARIS INI --}}
{{-- Jadi: --}}

{{-- ═══════════════════════════════════════════════════════ --}}
{{-- FILE LENGKAP petugas/aspirasi/index.blade.php          --}}
{{-- ═══════════════════════════════════════════════════════ --}}
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Keterangan Progres <span class="text-muted small">(opsional)</span></label>
                            <textarea class="form-control" id="inputKeteranganStatus" rows="2"
                                placeholder="Contoh: Sudah dikerjakan, menunggu bahan..." maxlength="500"></textarea>
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Catatan Progres Perbaikan</label>
                            <textarea class="form-control" id="inputProgres" rows="3"
                                placeholder="Jelaskan progres perbaikan yang sudah dilakukan..." maxlength="500"></textarea>
                        </div>
...
function loadData() {
    ...
    $.ajax({
        ...
        data: {
            page: currentPage, per_page: perPage, search: searchQuery,
            status: $('#filterStatus').val(), kategori: $('#filterKategori').val(),
        },
        success: renderTable,
...
function renderTable(res) {
    if (!res.data.length) {
        $('#tabelBody').html('<tr><td colspan="8" class="text-center text-muted py-4">Tidak ada laporan ditemukan.</td></tr>');
    ...
    res.data.forEach(function(d, i) {
        ...
        html += `<tr>
            ...
            <td>
                ...
                <span class="badge bg-soft-secondary text-secondary small">${d.role === 'siswa' ? 'Siswa' : 'Guru'}</span>
            </td>
            ...
            <td class="text-center">
                <button class="btn btn-sm btn-soft-info" onclick="lihatDetail(${d.id}, ${d.aspirasi_id})" title="Detail & Tindak Lanjut">
    ...
    $.get('{{ url("petugas/aspirasi") }}/' + id, function(d) {
        ...
        let feedbackHtml = d.feedback?.length
            ? d.feedback.map(f => `<div class="border rounded p-2 mb-2">
                <div class="small fw-semibold text-muted">${f.nama_pemberi} • ${f.created_at_fmt}</div>
        ...
        if (d.status === 'Proses') $('#selectStatusBaru').val('Selesai');
        else $('#selectStatusBaru').val('Proses');
        ...
        $('#detailBody').html(`
            <div class="row">
                <div class="col-md-4">
                    ...
                    <div class="mb-2"><small class="text-muted d-block">Role</small><span class="badge bg-soft-secondary text-secondary">${d.role === 'siswa' ? 'Siswa' : 'Guru'}</span></div>
                    ...
                    <div class="mb-2"><small class="text-muted d-block">Disetujui Oleh</small><span class="small">${d.disetujui_oleh}</span></div>
                    <div class="mb-3"><small class="text-muted d-block">Status</small>
                        <span class="badge ${statusMap[d.status]||'bg-secondary'} fs-6 px-3 py-2">${d.status}</span>
                    </div>
                    <small class="text-muted d-block mb-2">Foto Bukti</small>${fotoHtml}
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    <h6 class="text-muted border-bottom pb-2 mb-3">Catatan Progres</h6>
...
$('#btnUpdateStatus').on('click', function() {
    ...
    $.ajax({
        ...
        data: {
            _token:             '{{ csrf_token() }}',
            status:             $('#selectStatusBaru').val(),
            keterangan_progres: $('#inputKeteranganStatus').val(),
        },
        ...
        error: function(xhr) {
            let msg = xhr.responseJSON?.message || 'Gagal update status.';
            showToast(msg, 'danger');
        },
        complete: () => { $('#btnUpdateStatus').prop('disabled', false); $('#loaderStatus').addClass('d-none'); }
    });
});
...
$('#btnTambahProgres').on('click', function() {
    ...
    if (!ket) { alert('Keterangan progres tidak boleh kosong.'); return; }
    $(this).prop('disabled', true);
    ...
    $.ajax({
        ...
        method: 'POST',
        data: { _token: '{{ csrf_token() }}', keterangan_progres: ket },
        success: function(res) {
            showToast(res.message, 'success');
            $('#inputProgres').val('');
            lihatDetail(currentId, currentAspirasId);
        },
        error: () => showToast('Gagal menambah progres.', 'danger'),
        complete: () => { $('#btnTambahProgres').prop('disabled', false); $('#loaderProgres').addClass('d-none'); }
    });
});
